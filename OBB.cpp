...
void OBB::SetOrientations(const MyMatrix4x4& rotateMat) {
	for (int i = 0; i < 3; i++) {
...
void OBB::Draw(const MyMatrix4x4& viewProjectionMatrix, const MyMatrix4x4& viewportMatrix, uint32_t color) {
	...
	MyMatrix4x4 worldViewProjectionMatrix = MyMatrix4x4::Multiply(worldMat, viewProjectionMatrix);
	MyVector3 screen[8] = {};
